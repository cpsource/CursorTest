...
def logarithmic_func(x, a, b, c):
    ...
    with np.errstate(invalid='ignore', divide='ignore'):
        result = np.where(b * x > 0, a * np.log(b * x) + c, np.nan)
    return result
...
def fit_and_plot(x_data, y_data, filename):
    ...
    for func, name in functions:
        try:
            # Fit the curve with reasonable initial parameters
            if func == logarithmic_func:
                # For log function, ensure positive scaling
                initial_guess = [1.0, 1.0, 0.0]
                popt, _ = curve_fit(func, x_data, y_data, p0=initial_guess, maxfev=10000)
            else:
                popt, _ = curve_fit(func, x_data, y_data, maxfev=10000)
            
            # Calculate R-squared, handling NaN values
