...
class FacilityLocator:
    ...
    def setup_chains(self):
        """Setup LangChain for simple triangulation"""
        try:
            ...
            llm = OpenAI(
                ...
                max_tokens=200   # Enough for triangulation response
            )
            
            # Simple triangulation prompt template
            triangulation_template = """You are a detective and you are given distances from a crime scene. You are to determine the location of the crime scene in San Francisco.

Distances from crime scene:
{distances}

Reference location: {reference_location}

Based on these distances, determine the most likely location of the crime scene in San Francisco. Think of this as triangulation - you have multiple circles of the given radii, and you need to find where they intersect.

First, determine the crime scene location, then calculate the distance from that location to the reference location.

Respond in this format:
CRIME SCENE LOCATION: [specific address or intersection in San Francisco]
DISTANCE TO REFERENCE: [distance in tenths of miles]

Your analysis:"""

            distance_template = """Calculate the distance between two addresses in San Francisco in tenths of miles.

From: {from_address}
To: {to_address}

Return ONLY the distance as a number (e.g., 2.3). No units or explanations."""

            triangulation_prompt = PromptTemplate(
                input_variables=["distances", "reference_location"],
            ...
            print("✅ Simple triangulation chains initialized successfully")
    ...
    def parse_distances_simple(self, text):
        """Simple parsing - just extract distances, don't care about facility names"""
        print(f"📋 Simple distance parsing...")
        ...
        for line in lines:
            ...
            # Look for any number followed by "miles"
            distance_matches = re.findall(r'(\d+\.?\d*)\s*miles?\s*away', line, re.IGNORECASE)
        ...
        print(f"✅ Extracted {len(distances)} distances: {distances}")
        return distances
    ...
    def triangulate_simple(self, distances, reference_location):
        """Simple triangulation using just distances"""
        if not self.triangulation_chain:
            print("❌ Triangulation chain not available")
            return "Unable to triangulate - AI not configured"
        
        try:
            print(f"🎯 Starting simple triangulation with distances: {distances}")
            print(f"📍 Reference location: {reference_location}")
            
            # Build simple distances string
            distances_text = ""
            for i, distance in enumerate(distances, 1):
                distances_text += f"Distance {i}: {distance} miles\n"
            
            print(f"📋 Distances for AI:\n{distances_text}")
            ...
            print(f"🤖 AI triangulation response: '{result}'")
...
Your triangulated location analysis:"""
    ...
    def process_facilities(self, facilities_text, reference_address="1945 Broadway, San Francisco, CA"):
        """Simple processing - just extract distances and triangulate"""
        print(f"\n" + "="*60)
        print(f"=== SIMPLE TRIANGULATION STARTED ===")
        print(f"="*60)
        ...
        distances = self.parse_distances_simple(facilities_text)
        
        if len(distances) < 2:
            print(f"❌ Need at least 2 distances for triangulation")
            return "Need at least 2 distances for triangulation. Found distances like '1.64 miles away'."
        
        print(f"✅ Using {len(distances)} distances: {distances}")
        
        # Simple triangulation
        print(f"\n--- STEP 2: TRIANGULATION ---")
        triangulation_result = self.triangulate_simple(distances, reference_address)
        print(f"🎯 Triangulation result: '{triangulation_result}'")
        
        # Format simple results
        print(f"\n--- STEP 3: FORMAT RESULTS ---")
        formatted_results = self.format_simple_results(distances, triangulation_result, reference_address)
        
        print(f"\n" + "="*60)
        print(f"=== SIMPLE TRIANGULATION COMPLETED ===")
        print(f"="*60)
    ...
    def format_simple_results(self, distances, triangulation_result, reference_address):
        """Format simple triangulation results"""
        output = f"**🎯 CRIME SCENE TRIANGULATION**\n\n"
        
        output += f"**📍 Reference Location:** {reference_address}\n\n"
        
        output += f"**📏 Distance Constraints:**\n"
        for i, distance in enumerate(distances, 1):
            output += f"{i}. {distance} miles from crime scene\n"
        
        output += f"\n**🕵️ DETECTIVE ANALYSIS:**\n\n"
        output += triangulation_result
...
3.32 miles away"""
        ...
        with gr.Row():
            ...
            clear_input_btn = gr.Button("🗑️ Clear Input", size="sm")
